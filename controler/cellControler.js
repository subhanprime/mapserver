  ...
  try {
    ...
    if (type === "stripe") {
      ...
      if (charge && charge.status) {
        ...
        return res.json({ transaction, trx, message: "something went wrong" });
      }
    }
  } catch (err) {
...
//     { _id: data.nftId.toHexString() },
//     { paidStatusContract: true }
//   );
//   let updateTrx = await pendingTransactionsModal.findOneAndUpdate(
//     { nftId: data.nftId.toHexString() },
//     { transactionStatus: true }
//   );
//   if (update && updateTrx) return { status: true };
//   else {
//     return { status: false };
//   }
// };
// const checkTransactions = async (trans) => {
//   let tx = await provider.getTransactionReceipt(trans.transactionHash);
//   if (tx?.status) {
//     let { status } = updateNftStatus(trans);
//     // return { status: true }
//   } else {
//     return { status: false };
//   }
// };
// const updateTransactions = async (transaction) => {
//   for (let i = 0; i < transaction.length; i++) {
//     let { status } = checkTransactions(transaction[i]);
//     return { status };
//   }
// };
// let interval = 2000;
...
//   (async () => {
//     const getPendingTransactions = await pendingTransactionsModal.find({
//       transactionStatus: false,
//     });
//     if (getPendingTransactions.length > 0) {
//       let { status } = updateTransactions(getPendingTransactions);
//     }
//   })();
// }, interval);
